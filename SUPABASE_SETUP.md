...
CREATE TABLE public.likes (
  ...
  content_id uuid null default gen_random_uuid(),
  content_type text null,
  constraint likes_pkey primary key (id)
) TABLESPACE pg_default;
```

The `likes` table stores like/unlike events for arbitrary content. Each row
references a `content_type` (e.g. `'video'`, `'poem'`, `'comment'`) and a
`content_id` UUID. The application inserts a new record whenever a user
toggles a like; counts are calculated by querying `liked = true` rows.
